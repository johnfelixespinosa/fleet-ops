...
// FleetOps: Rails server lifecycle
...
function createWindow(): void {
  ...
  app.on('before-quit', () => {
    ...
    if (railsProcess) {
      ...
      railsProcess = null
      log('[rails] Stopped Rails server')
    }
  })
...
app.whenReady().then(async () => {
  ...
  // FleetOps: Start Rails server as managed child process
  railsProcess = spawn('bundle', ['exec', 'rails', 'server'], {
    cwd: API_DIR,
    env: { ...process.env, RAILS_ENV: 'development' },
    stdio: 'pipe',
  })
  railsProcess.stdout?.on('data', (data: Buffer) => log(`[rails] ${data.toString().trim()}`))
  railsProcess.stderr?.on('data', (data: Buffer) => log(`[rails] ${data.toString().trim()}`))
  railsProcess.on('exit', (code: number | null) => log(`[rails] exited with code ${code}`))
  log('[rails] Starting Rails server...')
  ...
  tray.setToolTip('FleetOps Copilot')
  tray.on('click', () => toggleWindow('tray click'))
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: 'Show FleetOps Copilot', click: () => showWindow('tray menu') },
